  ...
  <Action ref="actionController" :action="[
    {
      ...
      async onTrigger(sel) {
        ...
        createLoadingMessage('移动中')
          .bind(
            PromiseAll(sel.map(v => FavouriteDB.moveItem(v.item, cardKey, ...selectCardKeys)))
          )
        cancel()
      }
    },
    {
      ...
      async onTrigger(sel) {
        ...
        createLoadingMessage('复制中')
          .bind(PromiseAll(sel.map(v => FavouriteDB.upsertItem(v.item, ...selectCardKeys))))
        cancel()
      }
    },
    {
    ...
    <Layout title="">
      <template #rightNav>
        ...
        <NIcon size="calc(var(--spacing) * 6.5)" class="van-haptics-feedback rotate-90" color="var(--van-text-color-2)"
      ...
      <DcWaterfall class="h-full!" un-reloadable
        :source="{ data: PromiseContent.resolve(items), isEnd: true }" v-slot="{ item }" :col="1" :gap="0"
        :padding="0" :minHeight="0" :data-processor="v =>
