  ...
  public override async call(cfg: PluginConfig, setMeta: PluginBooterSetMeta): Promise<any> {
    ...
    if (search) {
      if (search.categories)
        for (const c of search.categories) uni.content.ContentPage.addCategories(plugin, c)
      if (search.tabbar) for (const c of search.tabbar) uni.content.ContentPage.addTabbar(plugin, c)
      if (search.hotPage) {
    ...
    if (cfg.config) {
      for (const config of cfg.config) Store.useConfig().$resignerConfig(config)
    }
    if (share) {
      for (const v of share.initiative ?? []) uni.content.ContentPage.share.set([plugin, v.key], v)
