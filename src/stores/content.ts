...
export const useContentStore = defineStore('content', helper => {
  ...
  const $createHistoryKey = helper.action((contentType_: uni.content.ContentType_, id: string, ep: string) => `${id}$${uni.content.ContentPage.contentPage.toString(contentType_)}$${ep}`, 'createHistoryKey')
  const $load = helper.action((contentType_: uni.content.ContentType_, id: string, ep: string, preload?: uni.content.PreloadValue, load: boolean = true, _offline: boolean = false) => {
    ...
    if (!history.has(itemId)) {
      var newIns = markRaw(new (uni.content.ContentPage.contentPage.get(contentType_)!)(preload, id, ep))
