...
vi.mock('../../../lib/driver/builtIn', () => ({
  isBuiltInPlugin: mocks.isBuiltInPlugin,
  isBuiltInPluginName: mocks.isBuiltInPluginName,
}))
vi.mock('../../../lib/driver/extensions', () => ({
  runtimeExtensions: { installers: { values: mocks.installers } },
}))
vi.mock('../../../lib/driver/loader', () => ({ loaders: mocks.loaders }))
vi.mock('../../../lib/driver/runtime', () => ({
  pluginRuntime: { uninstall: mocks.runtimeUninstall },
}))
vi.mock('../../../lib/driver/store', () => ({
  pluginStore: { $upsertArchives: mocks.upsertArchives },
}))
...
describe('plugin installation orchestration', () => {
  it('follows downloader redirects, clamps loader progress, and persists the archive', async () => {
    ...
    const { installPlugin } = await import('../../../lib/driver/install')
  ...
  it('installs only missing downloadable dependencies and honors app overrides', async () => {
    ...
    const { installDepends } = await import('../../../lib/driver/install')
  ...
  it('rejects unknown download handlers, unknown loaders, and reserved plugin ids', async () => {
    const { installFilePlugin, installPlugin } = await import('../../../lib/driver/install')
...
describe('plugin updates and removal', () => {
  it('updates through a redirected installer and the archive loader', async () => {
    ...
    mocks.loaders.push(loader)
    const { updatePlugin } = await import('../../../lib/driver/install')
  ...
  it('delegates uninstall to the runtime', async () => {
    const { uninstallPlugin } = await import('../../../lib/driver/install')
