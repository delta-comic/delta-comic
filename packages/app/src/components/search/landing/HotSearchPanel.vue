...
const emit = defineEmits<{ select: [section: ResolvedHotSearchSection, item: Content.SearchAim] }>()
const { t } = useI18n()
</script>
      ...
      <section v-for="section in sections" :key="section.id" class="grid gap-2.5">
        ...
        <div class="grid grid-cols-2 gap-x-3.5 gap-y-2 min-[720px]:grid-cols-3">
          <button
            ...
            :key="`${item.search.method}:${item.input}`"
            type="button"
          ...
          >
            <span class="dc-ellipsis">{{ item.input }}</span>
          </button>
        </div>
      </section>
    </template>
